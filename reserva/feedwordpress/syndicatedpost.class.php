...
//		array('[a-z]+', 'on[a-z]+'),
	...
	function store () {
		...
		add_action(
			/*hook=*/ 'transition_post_status',
			/*callback=*/ array(&$this, 'parse_media'),
			/*priority=*/ -10000, /* very early */
			/*arguments=*/ 3
		);
...
EOM;
			...
			if ( is_array($this->post) and isset($this->post['meta']) and is_array($this->post['meta']) ) :
				...
				foreach ( $this->post['meta'] as $key => $values ) :
					...
					FeedWordPress::diagnostic('syndicated_posts:meta_data', "Adding post meta-datum to post [$postId]: [$key] = ".FeedWordPress::val($value, /*no newlines=*/ true));
						...
						//add_post_meta($postId, 'pruebasa', 'pruebasass', /*unique=*/ false);///para probar
					endforeach;
				endforeach;
			endif;
		endif;
	...
	/////////////////////MAAAAINSSS
	/**
	 * SyndicatedPost::parse_media: parse and add rss media
	 * 
	 * @param string $new_status Unused action parameter.
	 * @param string $old_status Unused action parameter.
	 * @param object $post The database record for the post just inserted.
	 */
	function parse_media ($new_status, $old_status, $post) {
		global $wpdb;

	set_time_limit (20);

	if ($new_status!='inherit') : // Bail if we are creating a revision.
		FeedWordPress::diagnostic('syndicated_posts:parse_media', 'parseando media: {'.implode(", ", array_keys($this->post['meta'])).'}');
		
		if ( is_array($this->post) and isset($this->post['meta']) and is_array($this->post['meta']) ) :
			$postId = $post->ID;
		
			 $contenido = $this->content();
			 $contenido1 = $this->content();
			 
			 $resumen = limit_words(strip_tags($contenido),40);	
			 add_post_meta($postId,'resumen',$resumen, true); //agregar resumen a metadato

		
			//IMAGENES
			// We will search for the src="" in the post content JPGs
			$regular_expressionImg = '~src="[^"]*.jpg"~';
			preg_match_all( $regular_expressionImg, $contenido, $allpics);// WE will grab all the images from the post in an array $allpics using preg_match_all
			$NumberOfPics = count($allpics[0]);
	
			//search for PNGs
			if ($NumberOfPics == 0){	
				$regular_expressionImg = '~src="[^"]*.png"~';
				preg_match_all( $regular_expressionImg, $contenido, $allpics);
			}
			$NumberOfPics = count($allpics[0]);
		
			//search for GIFs
			if ($NumberOfPics == 0){	
				$regular_expressionImg = '~src="[^"]*.gif"~';
				preg_match_all( $regular_expressionImg, $contenido, $allpics);
			}
			$NumberOfPics = count($allpics[0]);
		...
		///LOAD IMAGES TO META
			...
			$imagenes = array();
			$nImagen = 00;
				foreach ($allpics[0] as $pic ) :
				

					$rawUrl = $pic;
					$quitar = 'src="';
					$cleanUrl = str_replace($quitar,"",$rawUrl);
					$quitar = '"';
					$cleanUrl = str_replace($quitar,"",$cleanUrl);
					add_post_meta($postId,'imagenesUrl',$cleanUrl, false); //agregar imagens a metadatos
					
					$imagenes[$nImagen]['url'] = (string)$cleanUrl;
					$url = (string)$cleanUrl;
					$size = getimagesize($url);
					$imagenes[$nImagen]['width'] = $size[0];
					$imagenes[$nImagen]['height'] = $size[1];
					$nImagen = $nImagen + 01;
				endforeach;
				
					add_post_meta($postId,'imagenes',$imagenes, false); //agregar imagens a metadatos
		...
		//VIDEOS
		//search for VIMEOS
			$regular_expressionVimeo = '~vimeo.com/(\d+)~';
			preg_match_all( $regular_expressionVimeo, $contenido, $allVimeos);
			$NumberOfVimeos = count($allVimeos[1]);
	   
			if ($NumberOfVimeos == 0){	
				$regular_expressionVimeo = '~video/(\d+)~';
				preg_match_all( $regular_expressionVimeo, $contenido, $allVimeos);
			}
			$NumberOfVimeos = count($allVimeos[1]);
			...
			$vimeos = array();
			$nVimeo = 00;
				foreach ($allVimeos[1] as $vim) :
				
				   
					
					$vimeoID = $vim;
					
					//AD VIMEO URL FOR FURTHER USE
						$vimeos_oembed_url[$nVimeo] = 'http://vimeo.com/api/oembed.xml?url=http%3A//vimeo.com/'.$vimeoID.'';
						add_post_meta($postId,'vimeos_oembed_url',$vimeos_oembed_url,false);
					
                    libxml_use_internal_errors(false);
					set_time_limit (45);
					
					
					$vimeoXml = simplexml_load_file('http://vimeo.com/api/oembed.xml?url=http%3A//vimeo.com/'.$vimeoID.'');
                    //add_post_meta($postId,'vimeo-N'.$nVideo.'-xml',$vimeoXml, false); //agregar id del video a metadatos
										
					///
					$xml = file_get_contents('http://vimeo.com/api/oembed.xml?url=http%3A//vimeo.com/'.$vimeoID.'');
					$xmlArray = xml2array($xml);
					
					
					foreach ($xmlArray as $xmlItem) :
						add_post_meta($postId,'vimeo-'.$nVimeo.'',$xmlItem,false);
					endforeach;
					////
					
					 $vimeos[$nVimeo] = $xmlArray;
				
					
					
					 $nVimeo = $nVimeo + 01;
				endforeach;	
				add_post_meta($postId,'vimeos',$vimeos,false);
			};
		///
			
			
		
			//search for YouTubes TODO
			//$regular_expressionYoutube = '~youtube-nocookie.com/v/[^?]+~';  //chekear esto
			//preg_match_all( $regular_expressionYoutube, $contenido, $allYoutubes);
			//$NumberOfYoutubes = count($allYoutubes[0]);
